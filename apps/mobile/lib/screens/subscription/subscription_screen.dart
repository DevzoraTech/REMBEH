...
import 'complete_sms_payment_screen.dart';
...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final bundlesPayload = results[1] as Map<String, dynamic>;
      final paymentsPayload = results[2] as Map<String, dynamic>;
      ...
      final payments = rawPayments is List
          ? rawPayments
                ...
                .where((p) => p.id.isNotEmpty && p.isSms)
                .toList()
          : <BillingPaymentRow>[];

      if (!mounted) return;
      BillingPaymentRow? pending;
      ...
      setState(() {
        ...
        _bundles = bundles;
        _smsPayments = payments;
        _pendingSmsPayment = pending;
  ...
  Future<void> _openBundlePayment(SmsBundleOption bundle) async {
    ...
    final paid = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => CompleteSmsPaymentScreen(
            ...
            child: _loading
                ...
                : RefreshIndicator(
                    ...
                    child: _tab == SubscriptionTab.sms
                            ...
                            : _SmsTab(
                                ...
                                onViewAllPayments: () {
                                  Navigator.of(context).push<void>(
                                    MaterialPageRoute(
                                      builder: (_) => _SmsPaymentsListScreen(
                                        payments: _smsPayments,
                                      ),
                                    ),
                                  );
                        ...
                        : _PlanTab(billing: _branchBilling),
                  ),
          ),
        ],
      ),
...
class _PlanTab extends StatelessWidget {
  const _PlanTab({required this.billing});
  ...
  Widget build(BuildContext context) {
    ...
    final planAmount = billing?['planAmount'] is num
        ? billing!['planAmount'] as num
        : num.tryParse('${billing?['planAmount']}') ?? 0;
    final currency = '${billing?['planCurrency'] ?? 'UGX'}';
    final periodEnd = DateTime.tryParse('${billing?['currentPeriodEnd'] ?? ''}');
    ...
    return ListView(
      ...
      children: [
        ...
        Container(
          ...
          child: const Text(
            'Plan renewals will use the same mobile money payment flow as SMS top-ups. For now, manage plan checkout on the web dashboard if needed.',
            style: TextStyle(
              color: slateText,
              fontSize: 13,
              fontWeight: FontWeight.w600,
...
class _SmsPaymentsListScreen extends StatelessWidget {
  const _SmsPaymentsListScreen({required this.payments});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: softIvory,
      appBar: AppBar(title: const Text('SMS payments')),
