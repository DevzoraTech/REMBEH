...
export function AgentDetailDrawer({
  ...
  const [tab, setTab] = useState<"collections" | "applications">("collections");
      ...
      <aside className="relative z-10 flex h-full w-full max-w-2xl flex-col border-l border-[var(--line)] bg-[var(--soft-ivory)] shadow-xl">
        <div className="flex items-start justify-between gap-3 border-b border-[var(--line)] px-4 py-3">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              {detail ? (
                <span
                  className={`mt-1 inline-block text-[10px] font-bold capitalize tracking-[0.08em] ${statusTone(detail.status)}`}
                >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          ...
          <div className="panel mb-3 flex flex-wrap items-end justify-between gap-3 px-3 py-3">
            <label className="grid gap-1 text-xs font-semibold capitalize text-slate-500">
              <span>day</span>
              <input
                type="date"
                value={date}
            ...
            <div className="flex gap-1" aria-label="Activity range">
              {(["today", "week", "all"] as const).map((value) => (
                <button
                  key={value}
                  type="button"
                  className={`h-8 px-2.5 text-xs font-semibold capitalize ${
                    range === value
          ...
          {activityLoading ? (
            ...
            collections.length === 0 ? (
              ...
              <ul className="divide-y divide-[var(--line)] border border-[var(--line)] bg-white">
                {collections.map((row) => (
                  <li key={row.id} className="hover:bg-[var(--soft-mist)]">
                    <Link
...
function TabButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-3 py-2 text-xs font-bold capitalize tracking-[0.06em] ${
        active
