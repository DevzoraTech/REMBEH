...
export default function OwnerCollectionsPage() {
  const state = useOwnerSession("/owner/collections");
  ...
  const collectionNotifications = useMemo<OwnerNotificationItem[]>(() => {
    const items: OwnerNotificationItem[] = [];
    if (filter === "collectedToday" && filtered.length === 0 && !loading) {
      items.push({
        id: "no-collections-today",
        title: "No collections today yet",
        detail: "Switch to All Payments to review the full repayment history.",
    ...
    return items;
  }, [filter, filtered.length, loading]);
