...
const OPENING = -6800;
const BRANCH_ID = '4aa65689-64fe-41cd-b2aa-e1608e34aa21';
...
async function main() {
  ...
  try {
    ...
    if (!branch) throw new Error('Bisheshe branch not found');
    ...
    if (!yesterdayOp) {
      ...
      yesterdayOp = await prisma.branchDailyOperation.create({
        data: {
          ...
          notes:
            '[COGLIM-BISHESHE] Cutover cash. Yesterday closing / today opening UGX -6,800.',
          closingNotes:
            '[COGLIM-BISHESHE] Confirmed office closing cash UGX -6,800.',
        },
      });
    } else {
      ...
      JSON.stringify(
        {
          ...
          today: todayOp
            ? {
                date: todayLabel,
                status: todayOp.status,
                previousClosingBalance: Number(
                  todayUpdate?.previousClosingBalance ??
                    todayOp.previousClosingBalance,
                ),
                openingFloatAvailable: Number(
                  todayUpdate?.openingFloatAvailable ??
                    todayOp.openingFloatAvailable,
                ),
