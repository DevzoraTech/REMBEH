...
describe('loan-agreement-docx.merger', () => {
  ...
  it('fills the real DOCX template without leaving merge tokens', async () => {
    ...
    expect(footer).toContain('Generated by: Rembeh Financial Software');
    expect(footer).toContain('20 July 2026');
    expect(footer).not.toContain('current_date');
    expect(footer).not.toContain('Generate by:');
    expect(footer).not.toContain('Finacial');
  });

  it('coalesces placeholders split across w:t runs', () => {
    const xml =
      '<w:t>Date: &lt;&lt;</w:t><w:proofErr w:type="spellStart"/>' +
      '<w:t>current_date</w:t><w:proofErr w:type="spellEnd"/>' +
      '<w:t>&gt;&gt;</w:t>';
    const coalesced = coalesceSplitPlaceholders(xml);
    expect(coalesced).toContain('&lt;&lt;current_date&gt;&gt;');
    const out = replaceMergeFieldsInXml(xml, { current_date: '20 July 2026' });
    expect(out).toContain('20 July 2026');
